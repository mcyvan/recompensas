...
if (isset($_SESSION['mensaje_registro_remision_error'])) {
    $mensaje_registro_remision_error = $_SESSION['mensaje_registro_remision_error'];
    unset($_SESSION['mensaje_registro_remision_error']);
} else {
    $mensaje_registro_remision_error = null;
}
...
if ($remision_activa && !$mensaje_registro_remision_error) {
...
<body class="layout-fixed sidebar-expand-lg sidebar-mini sidebar-collapse sidebar-mini-expand-feature bg-body-tertiary">
    <?php if ($mensaje_registro_remision_error): ?>
        <script>
            Swal.fire({
                title: "",
                text: <?= json_encode($mensaje_registro_remision_error, JSON_UNESCAPED_UNICODE) ?>,
                icon: "error"
            });
        </script>
    <?php endif; ?>
    <?php if ($mensaje_registro_remision_correcto): ?>
        <script>
            Swal.fire({
                title: "",
                text: <?= json_encode($mensaje_registro_remision_correcto, JSON_UNESCAPED_UNICODE) ?>,
                icon: "success"
            });
        </script>
    <?php endif; ?>
                                ...
                                <div class="card-body text-center">
                                    ...
                                    <div class="d-grid gap-2 col-6 mx-auto justify-content-center">
                                        <div class="row justify-content-center text-center">
                                            <a href="<?= $remision_activa
                                                ? 'remision_en_proceso.php?id=' . (int) $remision_activa['id_remision']
                                                : 'ingresar_tiempo_descarga.php' ?>" class="btn btn-outline-primary btn-square-lg mt-2">
